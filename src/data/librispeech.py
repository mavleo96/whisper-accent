...
class LibriSpeechDataModule(L.LightningDataModule):
    def __init__(
        ...
        # Create librispeech specific directory
        self.librispeech_dir = self.processed_dir / "librispeech"
        if not self.librispeech_dir.exists():
            self.librispeech_dir.mkdir(parents=True, exist_ok=True)

        model_dir_name = model_name.replace("/", "_").replace(".", "_")
        model_dir = self.librispeech_dir / model_dir_name
    ...
    def prepare_data(self):
        ...
        train_dataset = load_dataset(
            "librispeech_asr", "clean", split="train.100", cache_dir=self.cache_dir
        )
        val_dataset = load_dataset(
            "librispeech_asr", "clean", split="validation", cache_dir=self.cache_dir
        )
        test_dataset = load_dataset(
            "librispeech_asr", "clean", split="test", cache_dir=self.cache_dir
        )
...
if __name__ == "__main__":
    ...
    data_module = LibriSpeechDataModule(subset_mode=True, **cfg.data)
